...
export function PatientDashboard() {
  ...
  const [appointmentHighlights, setAppointmentHighlights] = useState([])
  const [appointmentHighlightsMode, setAppointmentHighlightsMode] = useState('upcoming')
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const upcomingAppointments = deriveUpcomingAppointments(appointmentsData)
      const recentAppointments = deriveRecentAppointments(appointmentsData)
      const shouldShowUpcoming = upcomingAppointments.length > 0

      setAppointmentHighlights((shouldShowUpcoming ? upcomingAppointments : recentAppointments).slice(0, 3))
      setAppointmentHighlightsMode(shouldShowUpcoming ? 'upcoming' : 'recent')
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          {
            label: stats?.upcomingAppointments > 0 ? 'Sắp tới' : 'Gần đây',
            value: stats?.upcomingAppointments > 0 ? stats.upcomingAppointments : appointmentHighlights.length,
            icon: Calendar,
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle>
                    {appointmentHighlightsMode === 'upcoming' ? 'Lịch hẹn sắp tới' : 'Lịch hẹn gần đây'}
                  </CardTitle>
                  <CardDescription>
                    {appointmentHighlightsMode === 'upcoming'
                      ? 'Các cuộc tư vấn đã lên lịch'
                      : 'Hiển thị các lịch hẹn mới nhất khi bạn chưa có lịch sắp tới'}
                  </CardDescription>
                </div>
