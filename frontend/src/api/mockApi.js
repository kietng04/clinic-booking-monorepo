...
export const chatbotApi = {
  getIntents: async () => {
    await delay(200)
    return [
      { id: 'BOOK_APPOINTMENT', name: 'Dat lich kham', description: 'Nguoi dung muon dat lich kham moi' },
      { id: 'SERVICE_PRICE', name: 'Gia dich vu', description: 'Nguoi dung hoi bang gia dich vu' },
      { id: 'CLINIC_ADDRESS', name: 'Dia chi phong kham', description: 'Nguoi dung hoi dia chi phong kham' },
      { id: 'UNKNOWN', name: 'Khong xac dinh', description: 'Khong xac dinh duoc intent' },
    ]
  },
  ...
  classify: async (question) => {
    ...
    if (q.includes('dat lich') || q.includes('hen kham')) {
      return {
        question,
        normalizedQuestion: q,
        intentId: 'BOOK_APPOINTMENT',
        intentName: 'Dat lich kham',
        confidence: 0.8,
        provider: 'RULE_BASED',
        fallbackUsed: false,
      }
    }
    if (q.includes('gia') || q.includes('phi')) {
      return {
        question,
        normalizedQuestion: q,
        intentId: 'SERVICE_PRICE',
        intentName: 'Gia dich vu',
        confidence: 0.75,
        provider: 'RULE_BASED',
        fallbackUsed: false,
      }
  ...
  chat: async (message) => {
    ...
    if (q.includes('dat lich')) {
      return {
        question: message,
        normalizedQuestion: q,
        answer: 'Ban vao muc Dat lich kham, chon bac si va khung gio phu hop, sau do xac nhan dat lich.',
        intentId: 'BOOK_APPOINTMENT',
        intentName: 'Dat lich kham',
        confidence: 0.8,
        classifierProvider: 'RULE_BASED',
        fallbackUsed: false,
        answerProvider: 'RULE_RAG',
        ragUsed: true,
        sources: [{ id: 'BOOKING_GUIDE', title: 'Huong dan dat lich', intentId: 'BOOK_APPOINTMENT', score: 0.8 }],
      }
    }
    return {
      question: message,
      normalizedQuestion: q,
      answer: 'Toi can them thong tin de tra loi chinh xac. Ban co the noi ro hon yeu cau?',
      intentId: 'UNKNOWN',
      intentName: 'Khong xac dinh',
      confidence: 0.2,
      classifierProvider: 'FALLBACK',
      fallbackUsed: true,
      answerProvider: 'RULE_TEMPLATE',
      ragUsed: false,
      sources: [],
    }
  },
