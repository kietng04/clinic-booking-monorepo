...
import { savePaymentBookingSummary } from '@/utils/paymentBookingSummary'
...
const STEPS = [
  ...
  { id: 2, name: 'Chọn ngày giờ', description: 'Chọn lịch tư vấn' },
...
const buildConsultationTopic = (doctor, notes) => {
  const normalizedNotes = String(notes || '').trim()
  if (normalizedNotes.length >= 5) {
    return normalizedNotes.slice(0, 160)
  }

  return `Tư vấn ${doctor?.specialization || 'đa khoa'}`
}

const buildConsultationDescription = ({
  doctor,
  selectedDate,
  selectedTime,
  selectedSlotLabel,
  ...
  [
    `Bác sĩ: ${doctor?.name || '--'}`,
    `Ngày tư vấn: ${formatLongDate(selectedDate)}`,
    `Khung giờ: ${selectedSlotLabel || selectedTime || '--'}`,
    `Bệnh nhân: ${selectedProfile?.fullName || '--'}`,
    `Số điện thoại: ${selectedProfile?.phone || patientPhone || '--'}`,
...
export function ConsultationRequest() {
  ...
  const bookingFee = useMemo(() => CONSULTATION_BOOKING_FEE, [])

  const bookingSummaryItems = useMemo(
    () => [
      { label: 'Ngày tư vấn', value: selectedDate ? formatShortDate(selectedDate) : '--' },
      { label: 'Khung giờ', value: selectedSlotLabel || selectedTime || '--' },
      { label: 'Bệnh nhân', value: selectedProfile?.fullName || '--' },
      { label: 'Phí tư vấn', value: formatCurrency(bookingFee) },
    ],
    [bookingFee, selectedDate, selectedProfile?.fullName, selectedSlotLabel, selectedTime],
  )
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const loadedDoctors = (Array.isArray(doctorResult.value) ? doctorResult.value : [])
          .map(normalizeDoctor)
          .filter((doctor) => doctor?.id && doctorSupportsConsultation(doctor))
        ...
        if (preselectedDoctorId) {
          ...
          if (!preselectedDoctor) {
            try {
              const fetchedDoctor = normalizeDoctor(await userApi.getUser(preselectedDoctorId))
              preselectedDoctor = doctorSupportsConsultation(fetchedDoctor) ? fetchedDoctor : null
            } catch (error) {
  ...
  const resolvePatientPhone = async () => {
    const profilePhone = String(selectedProfile?.phone || '').trim()
    if (profilePhone) return profilePhone

    const currentPhone = String(user?.phone || user?.phoneNumber || '').trim()
    if (currentPhone) return currentPhone

    if (!user?.id) return ''

    const latestUser = await userApi.getUser(user.id)
    const latestPhone = String(latestUser?.phone || latestUser?.phoneNumber || '').trim()

    if (latestPhone) {
      updateUser({ phone: latestPhone, phoneNumber: latestPhone })
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const patientPhone = await resolvePatientPhone()
      if (!patientPhone) {
        const message = 'Tài khoản chưa có số điện thoại để tạo thanh toán. Vui lòng cập nhật hồ sơ.'
        setSubmitError(message)
        showToast({ type: 'error', message })
        return
      }

      const consultationTopic = buildConsultationTopic(selectedDoctor, notes || reason)
      const consultationDescription = buildConsultationDescription({
        doctor: selectedDoctor,
        selectedDate,
        selectedTime: String(confirmedSlot.time || '').slice(0, 5),
        selectedSlotLabel:
          selectedSlotLabel ||
          buildSlotLabel(
            confirmedSlot.time,
      ...
      consultation = await consultationApi.createConsultation({
        doctorId: selectedDoctor.id,
        topic: consultationTopic,
        description: consultationDescription,
        fee: Number.isFinite(bookingFee) ? bookingFee : null,
      })

      const paymentAmount = Number.isFinite(bookingFee) && bookingFee >= 1000 ? Math.round(bookingFee) : 1000
      ...
      try {
        paymentResult = await paymentApi.createPayment({
          ...
          description: `Thanh toán lịch tư vấn #${consultation.id}`,
          paymentMethod: 'MOMO_WALLET',
          patientName: selectedProfile.fullName || user?.name || user?.fullName || 'Bệnh nhân',
          patientEmail: selectedProfile.email || user?.email || 'unknown@example.com',
          patientPhone,
          doctorId: selectedDoctor.id,
        ...
        if (paymentCreateError?.response?.status === 409) {
          paymentResult = await paymentApi.getPaymentByResource('CONSULTATION', consultation.id)
        } else {
      ...
      if (orderId) {
        savePaymentBookingSummary(orderId, {
          orderId,
          resourceType: 'CONSULTATION',
          ...
          slotLabel:
            selectedSlotLabel ||
            buildSlotLabel(confirmedSlot.time, confirmedSlot?.durationMinutes || confirmedSlot?.duration || slotDurationMinutes),
          ...
          patientPhone: selectedProfile.phone || patientPhone,
      ...
      showToast({ type: 'success', message: 'Đặt lịch tư vấn thành công.' })
      navigate(`/patient/consultations/${consultation.id}`)
    } catch (error) {
      if (consultation?.id) {
        try {
          await consultationApi.cancelDraftConsultation(consultation.id)
        } catch (rollbackError) {
          console.warn('Failed to rollback draft consultation', rollbackError)
        }
      }

      const errorMessage = extractApiErrorMessage(error, 'Không thể hoàn tất đặt lịch tư vấn.')
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <div className="flex flex-wrap items-center gap-2 text-sm text-[#6d8074]">
        ...
        <span className="font-semibold text-[#143c26]">Đặt lịch tư vấn</span>
      </div>
      ...
      <AnimatePresence mode="wait">
        {currentStep === 1 && (
          ...
          >
            ...
            <p className="mt-2 text-sm leading-6 text-[#587062]">
              Tìm bác sĩ theo tên hoặc chuyên khoa, sau đó tiếp tục chọn ngày và khung giờ tư vấn.
            </p>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-4 flex items-center justify-between gap-3">
                        <div className="text-sm font-semibold text-[#143c26]">{formatCurrency(CONSULTATION_BOOKING_FEE)}</div>
        ...
        {currentStep === 2 && (
          ...
          >
              ...
              <section className="rounded-[16px] border border-[#d7e2da] bg-white p-5 shadow-[0_8px_24px_rgba(15,23,42,0.04)] sm:p-6">
                <div className="grid gap-6 lg:grid-cols-[176px_minmax(0,1fr)]">
                  ...
                  <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_180px]">
                    ...
                    <div>
                      <div className="text-sm font-semibold text-[#476252]">Phí tư vấn</div>
                      <div className="mt-3 text-[32px] font-bold text-[#0f4f2a]">{formatCurrency(bookingFee)}</div>
                    </div>
                  </div>
                </div>
              </section>
            ...
            <section className="rounded-[16px] border border-[#d7e2da] bg-white p-5 shadow-[0_8px_24px_rgba(15,23,42,0.04)] sm:p-6">
              <h2 className="mt-2 text-[30px] font-bold text-[#143c26]">Chọn ngày và khung giờ tư vấn</h2>
              ...
              <div className="mt-6 rounded-[16px] border border-[#d7e2da] bg-white p-5">
                {scheduleLoading ? (
                  <div className="py-8">
                    <Loading text="Đang tải khung giờ tư vấn..." />
                  </div>
                ) : slotGroups.length > 0 ? (
        ...
        {currentStep === 3 && (
          ...
          >
            ...
            <aside className="rounded-[16px] border border-[#d7e2da] bg-white p-5 shadow-[0_8px_24px_rgba(15,23,42,0.04)] sm:p-6 lg:sticky lg:top-24 lg:self-start">
              <div className="text-xl font-semibold text-[#143c26]">Thông tin đặt lịch tư vấn</div>
          ...
          >
            <div className="space-y-6">
              ...
              <section className="rounded-[16px] border border-[#d7e2da] bg-white p-5 shadow-[0_8px_24px_rgba(15,23,42,0.04)] sm:p-6">
                ...
                <h2 className="mt-2 text-3xl font-bold text-[#143c26]">Kiểm tra thông tin trước khi thanh toán</h2>

                <div className="mt-6 rounded-[18px] border border-[#d7e2da] bg-[#fcfdfc] p-5">
                  <div className="flex flex-wrap items-start justify-between gap-4 border-b border-dashed border-[#d7e2da] pb-5">
                    <div>
                      <div className="text-xs font-semibold uppercase tracking-[0.22em] text-[#8a9b90]">Phiếu tư vấn</div>
                      <div className="mt-2 text-[28px] font-bold text-[#143c26]">Xác nhận lịch tư vấn</div>
                    </div>
                    <div className="rounded-[12px] border border-[#d7e2da] bg-white px-4 py-3 text-right">
                      <div className="text-xs font-semibold uppercase tracking-[0.16em] text-[#8a9b90]">Tổng thanh toán</div>
                      <div className="mt-1 text-[24px] font-bold text-[#0f4f2a]">{formatCurrency(bookingFee)}</div>
                    </div>
                  </div>
                  ...
                  <div className="mt-5 grid gap-x-6 gap-y-4 md:grid-cols-2">
                    ...
                    <div className="border-b border-[#edf2ee] pb-3">
                      <div className="text-xs font-semibold uppercase tracking-[0.14em] text-[#8a9b90]">Ngày tư vấn</div>
                  ...
                  <div className="mt-5 border-t border-dashed border-[#d7e2da] pt-5">
                    <div className="text-sm leading-7 text-[#667b6d]">
                      Bằng cách nhấn nút xác nhận, bạn đã đồng ý với các điều khoản và điều kiện đặt tư vấn.
                    </div>
                  </div>
                </div>

                <div className="mt-5 rounded-[14px] border border-[#d7e2da] bg-white p-4">
                  <div className="text-xs font-semibold uppercase tracking-[0.14em] text-[#8a9b90]">
                    Phương thức thanh toán
                  </div>
                  <div className="mt-2 text-base font-semibold text-[#143c26]">MoMo</div>
                  <div className="mt-1 text-sm text-[#667b6d]">
                    Bạn sẽ được chuyển đến MoMo để hoàn tất giao dịch.
                  </div>
                </div>
